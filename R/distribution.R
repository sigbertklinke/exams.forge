...
#  data("distributions", package="exams.forge")
  ...
  if (is.na(ind)) {
    ...
    ret$name <- the$distributions$r[ind]  
    discrete <- if (is.na(discrete)) the$distributions$discrete[ind] else discrete
  }
...
#  data("distributions", package="exams.forge")
  ...
  if (is.null(name)) {
    ind <- which(the$distributions$r==object$name)
    if (length(ind)) {
      type <- the$distributions$latex[ind]
...
#  data("distributions", package="exams.forge")
  ind   <- pmatch(name, the$distributions$r)
  return(object$name==the$distributions$r[ind])
} 
