...
craft <- function(..., .overwrite=FALSE) {
  l  <- substitute(...())
  ln <- names(l)
  if (ln[1]=='') ln[1] <- names(l)[1] <- "name" else stopifnot(ln[1]=="name")
  if (exists(l$name, envir=the) && !.overwrite) 
    stop(sprintf("Distribution template '%s' exist already", l$name))
  the$dist[[l$name]] <- l[!(duplicated(ln)|ln=='')]
  l$name
}
...
distribution <- function(name, ...) {
  d     <- the$dist[[name]]
  if (is.null(d))     
    stop(sprintf("Distribution template '%s' does not exist", name))
  args  <- list(...)
  nargs <- names(args)
...
# new
summary.distribution <- function(object, give.attr = FALSE, ...) {
  out <- capture.output(str(object, give.attr = give.attr, ...))
  cat(paste0(out, "\n"))
  invisible(out)
}
